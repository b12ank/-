...
from sklearn.metrics import mean_absolute_error, mean_squared_error  # [修改] 引入评估指标
import os
...
# 1.2 确定输入特征和目标
# HISTORY_INPUT = ["直调负荷", "联络线受电负荷", "风电总加", "光伏总加", "地方电厂发电总加", "竞价空间"]
# FUTURE_INPUT = ['日前统调负荷', '日前联络线负荷', '日前风电出力', '日前光伏出力', '日前地方电厂出力', '日前竞价空间']
HISTORY_INPUT = ["直调负荷", "联络线", "风电", "光伏", "竞价空间"]
FUTURE_INPUT = ["直调负荷_day_ahead", "联络线_day_ahead", "风电_day_ahead", "光伏_day_ahead", "竞价空间_day_ahead"]
INPUT = HISTORY_INPUT + FUTURE_INPUT
OUTPUT = '价差'
...
df1 = df_raw[HISTORY_INPUT + FUTURE_INPUT + [OUTPUT]]
print("原始数据缺失值：")
...
print(f"当前输入特征维度: {len(HISTORY_INPUT)}")
...
class ElectricityDataset(Dataset):
    def __init__(self, data, target, seq_len=24 * 30, pred_len=24, step=24):
        self.X = []
        self.y = []
        # 制作样本
        ...
        for i in range(0, len(data) - seq_len + 1, step):
            # 这里修改X的取样!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            patch_1 = data[i: i + seq_len - pred_len, :len(HISTORY_INPUT)]  # 历史特征数据
            patch_2 = data[i + seq_len - pred_len: i + seq_len, len(HISTORY_INPUT):]  # 未来预测特征数据
            X = np.concatenate((patch_1, patch_2), axis=0)  # 拼接历史90 + 未来30天预测值
            self.X.append(X)
            # y是未来30天的预测目标值
            self.y.append(target[i + seq_len - pred_len: i + seq_len])
...
for output_len in output_len_list:
    PRED_LEN = output_len  # 一次输出多少点
    for input_len in input_len_list:
        SEQ_LEN = 24 * input_len + PRED_LEN
        train_point = 7320  # 前305天7320条用于训练，后61天(11.1-1:00 ~ 12.31-24:00)1464条用于测试
        # train_point = 24864  # 1036天
        ...
        if len(test_dataset) == 0:
            print("⚠️ (输入{input_len}天, 输出{output_len}点)测试集数据不足以进行 step=24 的采样")

        train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True)  # 训练可以 Shuffle
        test_loader = DataLoader(test_dataset, batch_size=1, shuffle=False)  # 测试不要 Shuffle

        print(f"(输入{input_len}天, 输出{output_len}点)训练集样本数: {len(train_dataset)}")
        print(f"(输入{input_len}天, 输出{output_len}点)测试集样本数: {len(test_dataset)}")
        ...
        if TRAIN_OR_INFERENCE == 0:
            ...
            model = PricePredictor(input_size=len(HISTORY_INPUT), output_size=PRED_LEN)
            ...
            print(f"(输入{input_len}天, 输出{output_len}点)开始训练...")
            ...
            torch.save(model.state_dict(), os.path.join(model_dir, f"LSTM({input_len}天输出最后{output_len}点).pth"))
            print(f"保存模型到:{model_dir}\LSTM({input_len}天输出最后{output_len}点).pth")
        ...
        # 直接读取保存模型参数
            ...
            MODEL_PATH = os.path.join(model_dir, f"LSTM({input_len}天输出最后{output_len}点).pth")
        ...
        model.eval()  # 关闭dropout，不更新模型
        all_preds = []
        all_trues = []

        print("正在对测试集进行全量预测...")
        with torch.no_grad():  # 不记录计算过程，只给结果
        ...
        # 可视化最后14天的预测情况
        plt.plot(flat_trues[-15 * 24:], label=f'真实{OUTPUT}', color='blue', alpha=0.7)
        plt.plot(flat_preds[-25 * 24:], label=f'预测{OUTPUT}', color='red', alpha=0.7, linestyle='--')
        plt.title(f'({input_len}天输出最后{output_len}点){OUTPUT}预测对比 (MAE: {mae:.2f})')
        plt.xlabel('时间 (Hours)')
        plt.ylabel(OUTPUT)
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(os.path.join(picture_dir, f'LSTM_{input_len}天输出最后{output_len}点.png'))
        print(f"预测图保存到:{picture_dir}\LSTM_{input_len}天输出最后{output_len}点.png")
        # try:
        #     plt.show()
...
# 保存路径设置
save_csv_path = os.path.join(save_dir, "不同输入输出长度对比1.csv")
# 确保目录存在
